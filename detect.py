...
class ObjectDetectorCascadeOpenCV(ObjectDetector):
    ...
    def run_profile_raw(
        ...
        for angle in range(-90, 91, 45):
            ...
            min_h = int(
                max(rotated_img.shape[0] / self.min_height_dec, self.min_height_thresh))
            min_w = int(
                max(rotated_img.shape[1] / self.min_width_dec, self.min_width_thresh))
    ...
    def run(self, image_file):
        ...
        min_h = int(
            max(img.shape[0] / self.min_height_dec, self.min_height_thresh))
        min_w = int(
            max(img.shape[1] / self.min_width_dec, self.min_width_thresh))
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        faces = self.face_cascade.detectMultiScale(
